­r‡^Ñf¥–Ø¦{NìyÊ'vÃ®¶›­import assert from "node:assert/strict";
...
test("computes SLA state and queue metrics from ticket outcomes", () => {
  ...
  assert.equal(getSlaState({ ...seedTickets[0], status: "open", slaDueAt: "2026-07-14T11:00:00.000Z" }, now), "breached");
  assert.equal(getSlaState({ ...seedTickets[0], status: "open", slaDueAt: "2026-07-14T13:00:00.000Z" }, now), "due-soon");
  assert.equal(getSlaState({ ...seedTickets[0], status: "resolved" }, now), "resolved");

  const metrics = getQueueMetrics(seedTickets, now);
  assert.equal(metrics.total, seedTickets.length);
...
test("records assignment, status changes, and replies in an audit trail", () => {
  ...
  const replied = addReply(assigned, "Thanks â€” we are checking this now.", "Jon Bell", "2026-07-14T12:05:00.000Z");
  assert.equal(replied.activity.at(-1).type, "reply");
  assert.equal(replied.activity.at(-1).body, "Thanks â€” we are checking this now.");
});
